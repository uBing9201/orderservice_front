...
const ProductList = ({ pageTitle }) => {
  ...
  const pageSize = 15; // 한 페이지당 노출시킬 아이템 수
  ...
  useEffect(() => {
    ...
    // 상태가 바뀌면 그 때 백엔드로 요청을 보낼 수 있게 로직 분리
    if (currentPage > 0) loadProduct();
  }, [currentPage]);

  // 상품 목록을 백엔드에 요청하는 함수
  const loadProduct = async () => {
    // 아직 로딩중 이라면 or 이미 마지막 페이지라면 더이상 진행하지 말아라
  ...
  const handleAddToCart = () => {
    ...
    // key값만 뽑아서 selected에 들어있는 상품 중에 false 다 빼고 true인 것만 담기
    const filtered = selectedProduct.filter((key) => selected[key]);

    // 사용자가 선택한 수량까지 파악해서 장바구니에 넣어 줍니다.
